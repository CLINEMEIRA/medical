    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load 
        ...
        If Not Page.IsPostBack Then 
            If Request.QueryString("key1") Is Nothing And Request.QueryString("key2") Is Nothing Then 
                MedicalLib.Common.Sort = "WORKER_CD" 
            End If 
...
#Region " ボタン "
    ...
    Protected Sub btnSearch_ServerClick(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSearch.Click
        Session("SortExpression") = "WORKER_CD"
...
#Region " ユーザー関数 "
    ...
    Private Sub GridBind() 
        Try 
            ...
            sQuery = "SELECT WORKER_CD,WORKER_NM,WORKER_ABB,(WORKER_MST.SHOP||'：'||SHOP_NM) SHOP,PASSWORD,(ADMIN_LV||'：'||ITEM_NM) ADMIN_LV " 
            sQuery += "FROM WORKER_MST,SHOP_MST,GENERAL_PURPOSE_TBL " 
            sQuery += "WHERE WORKER_MST.SHOP = SHOP_MST.SHOP(+) " 
            sQuery += "AND WORKER_MST.ADMIN_LV = GENERAL_PURPOSE_TBL.TELL_CD(+) " 
            sQuery += "AND GENERAL_PURPOSE_TBL.INFO_KB = 'MS001' " 
            sQuery += "AND WORKER_NM LIKE :K01" 
            _oDatabase.CreateParam("K01", "%" + txtName.Text.Trim() + "%") 
            If Not ddlShop.SelectedValue.Trim.Equals("") Then 
                sQuery += " AND WORKER_MST.SHOP = :K02" 
