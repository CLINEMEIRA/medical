...
Partial Class master_mdsrc270
    Inherits System.Web.UI.Page

    Private Const PROGRAM_ID As String = "MDSRC270"
    Private Const PROGRAM_TITLE As String = "源製品在庫検索"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not Page.IsPostBack Then
                lblBuhin.Text = Request.QueryString("oya").Trim()
                txtSiji.Text = Request.QueryString("siji").Trim()
                MedicalLib.Common.ListSort = "PRIORITY_SORT, CATALOG_NO, INDICATE_NO"
...
#Region " 画面関連 "
    ...
    Private Sub DisplayList()
        ...
        Try
            sQuery += "SELECT PS.INDICATE_NO, PS.COMPANY_NO, PS.CATALOG_NO"
            sQuery += "     , PS.F_STOCK_AMT, PS.RSTOCK_AMT"
            sQuery += "     , PC.PRIORITY AS PRIORITY_SHOW"
            sQuery += "     , NVL2(PC.PRIORITY, PC.PRIORITY, 1000) AS PRIORITY_SORT"
            sQuery += "  FROM PARTS_STOCK_TBL PS"
            sQuery += "     , (SELECT * FROM O_PARTS_CONSIST_MST WHERE PARTS_NO = :K01) PC"
            sQuery += " WHERE PS.F_STOCK_AMT > 0"
            sQuery += "   AND PS.RSTOCK_AMT > 0"
            sQuery += "   AND PS.C_NO = 'Z9'"
            sQuery += "   AND PS.CATALOG_NO = PC.O_PARTS_NO(+)"
            sQuery += "   AND SUBSTR(PS.CATALOG_NO,1,:K10) >= :K11 AND SUBSTR(PS.INDICATE_NO,1,:K20) >= :K21"
            sQuery += " ORDER BY PRIORITY_SORT, PS.INDICATE_NO, PS.COMPANY_NO"

            _oDatabase.CreateParam("K01", Me.lblBuhin.Text)
            _oDatabase.CreateParam("K10", GetKeyLength(Me.txtCatalog.Text.Trim))
            _oDatabase.CreateParam("K11", GetKey(Me.txtCatalog.Text.Trim))
            _oDatabase.CreateParam("K20", GetKeyLength(Me.txtSiji.Text.Trim))
            _oDatabase.CreateParam("K21", GetKey(Me.txtSiji.Text.Trim))
