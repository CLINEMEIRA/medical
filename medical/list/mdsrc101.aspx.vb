    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not Page.IsPostBack Then
                lblCode.Text = Request.QueryString("zairyo")
                txtLotNo.Text = Request.QueryString("lotno")

                If Not Page.Request.QueryString("IsZero") Is Nothing Then
                    MedicalLib.Common.ListSort = "STOCK_MINUS"
                Else
                    MedicalLib.Common.ListSort = "LOT_NO"
                End If

                scrManager.SetFocus(txtLotNo)
            Else
...
#Region " 画面関連 "
    ...
    Private Sub DisplayList()
        ...
        Try
            '**** 2010/04 条件追加 在庫量0も表示？ ST ****
            sQuery += "SELECT LOT_NO"
            sQuery += "     , REN_NO"
            sQuery += "     , STOCK_WAT"
            sQuery += "     , STOCK_WAT * -1 AS STOCK_MINUS"
            sQuery += "     , F_STOCK_WAT"
            sQuery += "     , MORTGAGE_AMT"
            sQuery += "  FROM MATL_STOCK_TBL"
            sQuery += " WHERE MATL_CD = :K01"
            sQuery += "   AND SUBSTR(LOT_NO,1,:K10) >= :K11"
            sQuery += "   AND STOCK_WAT > 0"
            If Not Page.Request.QueryString("IsZero") Is Nothing Then
                sQuery += " UNION ALL "
                sQuery += "SELECT LOT_NO"
                sQuery += "     , REN_NO"
                ...
                sQuery += "     , MORTGAGE_AMT"
                sQuery += "  FROM MATL_STOCK_TBL"
                sQuery += " WHERE MATL_CD = :K02"
                sQuery += "   AND SUBSTR(LOT_NO,1,:K20) >= :K21"
                sQuery += "   AND STOCK_WAT = 0"
                'sQuery += " ORDER BY MATL_CD, LOT_NO"
                _oDatabase.CreateParam("K02", lblCode.Text.Trim())
                _oDatabase.CreateParam("K20", GetKeyLength(txtLotNo.Text))
                _oDatabase.CreateParam("K21", GetKey(txtLotNo.Text))
            Else
                sQuery += " ORDER BY MATL_CD, LOT_NO"
            End If
            'sQuery = "SELECT LOT_NO,REN_NO,STOCK_WAT,F_STOCK_WAT,MORTGAGE_AMT "
            'sQuery += "FROM MATL_STOCK_TBL "
            'sQuery += "WHERE MATL_CD = :K01 AND SUBSTR(LOT_NO,1,:K10) >= :K11 AND STOCK_WAT > 0 "
            'sQuery += " ORDER BY MATL_CD,LOT_NO"
            '**** 2010/04 条件追加 在庫量0も表示？ ED ****
            _oDatabase.CreateParam("K01", lblCode.Text.Trim())
            _oDatabase.CreateParam("K10", GetKeyLength(txtLotNo.Text))
            _oDatabase.CreateParam("K11", GetKey(txtLotNo.Text))
